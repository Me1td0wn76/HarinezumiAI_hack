...
export function EventCard({ event }: { event: EventSummaryDto }) {
  return (
    ...
    // こうすることで、中の主催者名だけ別のリンク先を持たせても <a> の入れ子にならない。
    <article className="card relative transition hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(0,0,0,0.09)]">
      <Link href={`/events/${event.id}`} className="absolute inset-0" aria-label={event.title} />

      <div className="pointer-events-none mb-2 flex items-start justify-between gap-3">
      ...
      <div
        className={`pointer-events-none mt-3 flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold ${
      ...
      <p className="mt-2 text-xs text-subtle">
        主催:{" "}
        {/* relative + z-10 で、背景に敷いたカード全体のリンクより手前に出す */}
        <Link href={`/users/${event.organizer.id}`} className="relative z-10 underline hover:text-foreground">
