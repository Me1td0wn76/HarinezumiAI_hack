...
| POST | `/events` | 必須 | LT会作成（`CreateEventRequest`、`tags` は最大 5 個、`format` 省略時は ONLINE、`webhookUrl` は任意）。Discord 通知 + 主催者以外の全ユーザーにアプリ内通知（主催者をブロックした人は除く） | `EventDetailDto` |
...
| GET | `/notifications` | 必須 | 自分宛てのアプリ内通知（新しい順、30件ずつ）。`?cursor=<nextCursor>` で続きを取る | `PageDto<NotificationDto>` |
