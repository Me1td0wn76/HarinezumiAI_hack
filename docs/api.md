...
| GET | `/schedule` | 任意 | みんなのカレンダー。`?from=&to=`（ISO 8601、`[from, to)`、最大 62 日）に候補日（調整中）か開催日（決定済み）がある公開中のLT会の日程（開始順）。終了・非表示のLT会と、ログイン時はブロックした相手のLT会を除く。1回に最大 500 件のLT会 | `PublicScheduleItemDto[]` |
...
- `entries` は最大 500 件。ログインしていれば、ブロックした相手の表明を除く（コメント欄と同じ）
- `EventDetailDto.myEntry` は閲覧者自身の表明（未ログイン・未表明は null。500 件を超えるLT会でも含める）
...
| `PUT` / `DELETE /events/:id/entry` | 20 回 / 分 |
