...
export class PasswordResetService {
  ...
  async confirm(dto: ConfirmPasswordResetDto): Promise<void> {
    const record = await this.resets.findByHash(hashToken(dto.token));
    if (!record || record.expiresAt < new Date()) {
      throw new BadRequestException('リンクが無効か期限切れです。もう一度再設定をリクエストしてください');
    }
    await this.resets.resetPassword(record.userId, await hash(dto.password, BCRYPT_ROUNDS));
  }
}
